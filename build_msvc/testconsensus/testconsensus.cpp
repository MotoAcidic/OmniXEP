...
// xep includes.
#include <..\src\script\xepconsensus.h>
...
int main()
{
    std::cout << "xepconsensus version: " << xepconsensus_version() << std::endl;
    ...
    auto op0Result = xepconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, xepconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
