// Copyright (c) 2014-2020 The Xep Core developers
...
#define XEP_TIMEDATA_MAX_SAMPLES 200
...
void AddTimeData(const CNetAddr& ip, int64_t nOffsetSample)
{
    ...
    if (setKnown.size() == XEP_TIMEDATA_MAX_SAMPLES)
        return;
    if (!setKnown.insert(ip).second)
        return;

    // Add data
    static CMedianFilter<int64_t> vTimeOffsets(XEP_TIMEDATA_MAX_SAMPLES, 0);
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                for (const int64_t nOffset : vSorted) {
                    if (nOffset != 0 && nOffset >= -DEFAULT_MAX_TIME_ADJUSTMENT && nOffset <= DEFAULT_MAX_TIME_ADJUSTMENT) fMatch = true;
                }
