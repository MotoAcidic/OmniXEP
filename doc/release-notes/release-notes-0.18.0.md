Xep Core version 0.18.0 is now available from:

  <https://xepcore.org/bin/xep-core-0.18.0/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://xepcore.org/en/list/announcements/join/>
...
`/Applications/Xep-Qt` (on Mac) or `xepd`/`xep-qt` (on
Linux).
...
Xep Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Xep Core on unsupported systems.

Xep Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Xep Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/xep/xep/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://xepcore.org/en/contact/) the Xep Core
...
The systemd init file (`contrib/init/xepd.service`) has been changed
to use `/var/lib/xepd` as the data directory instead of
`~xep/.xep`. This change makes Xep Core more consistent with
...
`datadir` option in `/etc/xep/xep.conf` will have no effect.
...
- A new short [document](https://github.com/xep/xep/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/xep/xep/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/xep/xep/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/xep/xep/blob/master/doc/xep-conf.md)
  about the `xep.conf` file describes how to use it to configure
  Xep Core.

- A new document introduces Xep Core's BIP174 [Partially-Signed
  Xep Transactions
  (PSBT)](https://github.com/xep/xep/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/xep/xep/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Xep-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Xep Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/xep/xep/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/xep/xep/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Xep Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Xep Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/xep/xep/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/xep/xep/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart xepd with the `-deprecatedrpc=generate`
...
- A new `xep-wallet` tool is now distributed alongside Xep
...
This section describes planned changes to Xep Core that may affect
other Xep software and services.

- Since version 0.16.0, Xep Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Xep Core 0.20 (expected about
  a year after 0.18), Xep Core will default to native segwit
  ...
  to bech32 addresses, and if the Xep Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Xep Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Xep Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Xep Core
...
- A sub-project of Xep Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Xep Core.  See their [project
  page](https://github.com/xep-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Xep Core's own implementation, although entropy
  gathered by Xep Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Xep Core a
...
- On macOS, Xep Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `xep_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libxep_server.a` (jnewbery)
...
- #13926 [Tools] xep-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like xepd (MarcoFalke)
...
- #14521 qt, docs: Fix `xep-qt -version` output formatting (hebasto)
...
- #11625 Add XepApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libxep_server` (laanwj)
...
- #15154 Configure: xep-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/xep-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Xep Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libxep_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in xep-util-test.py (isghe)
...
- #12775 Integration of property based testing into Xep Core (Christewart)
...
- #12246 Bugfix: Only run xep-tx tests when xep-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_xep` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_xep` (MarcoFalke)
...
- #14186 xep-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_xep.conf` message on startup if conf file exists (leishman)
...
- #12255 Update xep.service to conform to init.md (dongcarl)
...
- #14497 Add doc/xep-conf.md (hebasto)
...
- #15354 Add missing `xep-wallet` tool manpages (MarcoFalke)
...
- #15765 correct xepconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support xep folder on OSX (JimmyMow)
