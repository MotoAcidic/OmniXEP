...
Xep Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Xep Core is a multi-threaded application, and deadlocks or other
...
Xep Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Xep Core, where everyone uses
...
pay attention to for reviewers of Xep Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Xep Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `XEP_FOO_BAR_H`, e.g.

```c++
#ifndef XEP_FOO_BAR_H
#define XEP_FOO_BAR_H
...
#endif // XEP_FOO_BAR_H
```
...
Some of these are maintained by active developers of Xep Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Xep Core so that they can be integrated
...
file descriptors, it will cause problems with Xep's `select()` loop, because
...
compatibility between nodes. This happened in Xep 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Xep's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Xep Core, the wallet was always
